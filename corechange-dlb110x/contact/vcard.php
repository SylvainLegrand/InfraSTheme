...
$v->setName($contact->lastname, $contact->firstname, "", $langs->transnoentitiesnoconv('Civility'.$contact->civility_code));	// InfraS change
$v->setFormattedName($contact->getFullName($langs, 1, -1));	// InfraS change
...
$adr1	= str_replace('...', '', dolGetFirstLineOfText($contact->address));	// InfraS add
$adr2	= trim(str_replace($adr1, '', dol_string_nohtmltag($contact->address)));	// InfraS add
$v->setAddress("", "", $adr1.($adr2 ? "\n".$adr2 : ''), $contact->town, "", $contact->zip, ($contact->country_code?$contact->country:''), "TYPE=WORK;POSTAL");	// InfraS change
$v->setLabel("", "", $adr1.($adr2 ? "\n".$adr2 : ''), $contact->town, "", $contact->zip, ($contact->country_code?$contact->country:''), "TYPE=WORK");	// InfraS change
...
if ($company->id)
{
	...
	if (! $contact->zip)
	{
		$adr1	= str_replace('...', '', dolGetFirstLineOfText($company->address));
		$adr2	= trim(str_replace($adr1, '', dol_string_nohtmltag($company->address)));
		$v->setAddress("", "", $adr1.($adr2 ? "\n".$adr2 : ''), $company->town, "", $company->zip, ($company->country_code?$company->country:''), "TYPE=WORK;POSTAL");
		$v->setLabel("", "", $adr1.($adr2 ? "\n".$adr2 : ''), $company->town, "", $company->zip, ($company->country_code?$company->country:''), "TYPE=WORK");
	}
	// InfraS change end
