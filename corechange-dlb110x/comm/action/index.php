...
if (!empty($conf->use_javascript_ajax))	// If javascript on
{
	...
	// Local calendar
	$s .= '<div class="nowrap clear minheight20"><input type="checkbox" id="check_mytasks" name="check_mytasks" checked disabled> '.$langs->trans("LocalAgenda").' &nbsp; </div>';	// InfraS change delete inline-block
	...
	if (is_array($showextcals) && count($showextcals) > 0)
	{
		...
		foreach ($showextcals as $val)
		{
			...
			$s .= '<div class="nowrap"><input type="checkbox" id="check_ext'.$htmlname.'" name="check_ext'.$htmlname.'" checked> '.$val['name'].' &nbsp; </div>';	// InfraS change delete inline-block
		}
	}

	// Birthdays
	$s .= '<div class="nowrap"><input type="checkbox" id="check_birthday" name="check_birthday"> '.$langs->trans("AgendaShowBirthdayEvents").' &nbsp; </div>';	// InfraS change delete inline-block
...
print load_fiche_titre($s, $link.' &nbsp; &nbsp; '.$nav, 'Calendars.png', 0, 0, 'tablelistofcalendars');	// InfraS ajout "Calendars.png"
