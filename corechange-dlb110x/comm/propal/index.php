...
print load_fiche_titre($langs->trans("ProspectionArea"), '', 'proposal');	// InfraS change commercial by proposal
...
{
	$sql = "SELECT c.rowid, c.ref, c.fk_projet, s.nom as socname, s.rowid as socid, s.canvas, s.client";	// InfraS add ", c.fk_projet"
	...
	if ($resql)
	{
		...
		if ($num)
		{
			...
			$langs->load("propal");
			print '<td colspan="3">'.$langs->trans("DraftPropals").' <a href="'.DOL_URL_ROOT.'/comm/propal/list.php?viewstatut=0"><span class="badge">'.$num.'</span></a></td></tr>';	// InfraS change colspan 2 to 3
			...
			while ($i < $nbofloop)
			{
				...
				$staticproject		= $projectstatic->getNomUrl(1);	// InfraS add
				print '<td>'.($obj->fk_projet > 0 ? $staticproject : '').'</td>';	// InfraS add
				print '</tr>';
				$i++;
			}
			if ($num > $nbofloop)
			{
				print '<tr class="liste_total"><td colspan="3" class="right">'.$langs->trans("XMoreLines", ($num - $nbofloop))."</td></tr>";	// InfraS change colspan 2 to 3
			}
			elseif ($total > 0)
			{
				print '<tr class="liste_total"><td class="right" colspan="2">'.$langs->trans("Total").'</td><td class="right">'.price($total)."</td></tr>";	// InfraS change add colspan="2"
			}
			print "</table>";
			print "</div><br>";
		}
...
$sql = "SELECT c.rowid, c.entity, c.ref, c.fk_projet, c.fk_statut, s.nom as socname, s.rowid as socid, s.canvas, s.client,";	// InfraS add ", c.fk_projet"
...
if ($resql)
{
	...
	print '<table class="noborder centpercent">';
	print '<tr class="liste_titre">';
	print '<td colspan="5">'.$langs->trans("LastModifiedProposals", $max).'</td></tr>';	// InfraS change colspan 4 to 5
...
if (!empty($conf->propal->enabled) && $user->rights->propale->lire)
{
	...
	$sql.= ", p.rowid as propalid, p.entity, p.total as total_ttc, p.total_ht, p.ref, p.fk_projet, p.fk_statut, p.datep as dp, p.fin_validite as dfv";	// InfraS add ", p.fk_projet"
	$sql .= " FROM ".MAIN_DB_PREFIX."societe as s";
	$sql .= ", ".MAIN_DB_PREFIX."propal as p";
	if (!$user->rights->societe->client->voir && !$socid) $sql .= ", ".MAIN_DB_PREFIX."societe_commerciaux as sc";
	$sql .= " WHERE p.fk_soc = s.rowid";
	...
	if ($result)
	{
		...
		if ($num > 0)
		{
			print '<div class="div-table-responsive-no-min">';
			print '<table class="noborder centpercent">';
			print '<tr class="liste_titre"><td colspan="6">'.$langs->trans("ProposalsOpened").' <a href="'.DOL_URL_ROOT.'/comm/propal/list.php?viewstatut=1"><span class="badge">'.$num.'</span></a></td></tr>';	// InfraS change colspan 5 to 6
			...
			while ($i < $nbofloop)
			{
				...
				$staticproject		= $projectstatic->getNomUrl(1);	// InfraS add
				print '<td>'.($obj->fk_projet > 0 ? $staticproject : '').'</td>';	// InfraS add
				print '<td>';	// InfraS delete class right
			...
			elseif ($total > 0)
			{
				print '<tr class="liste_total"><td colspan="4" class="right">'.$langs->trans("Total")."</td><td align=\"right\">".price($total)."</td><td>&nbsp;</td></tr>";	// InfraS change colspan 3 to 4
			}
			print "</table>";
			print "</div><br>";
		}
