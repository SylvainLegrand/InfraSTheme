...
//	print $object->tva_intra;	// InfraS change
	...
	if (!empty($conf->contrat->enabled) && $user->rights->contrat->lire)
	{
		...
		if ($resql)
		{
			...
			if ($num > 0)
			{
			    ...
			    print '<tr class="liste_titre">';
				print '<td colspan="4"><table width="100%" class="nobordernopadding"><tr><td>'.$langs->trans("LastContracts",($num<=$MAXLIST?"":$MAXLIST)).'</td>';	// InfraS Change
	...
	{
		$sql = "SELECT s.nom, s.rowid, f.rowid as id, f.ref, f.fk_statut, f.duree as duration, f.dateo as startdate";	// InfraS Change
		...
		if ($resql)
		{
			...
			if ($num > 0)
			{
			    ...
			    print '<tr class="liste_titre">';
				print '<td colspan="4"><table width="100%" class="nobordernopadding"><tr><td>'.$langs->trans("LastInterventions", ($num <= $MAXLIST ? "" : $MAXLIST)).'</td><td class="right"><a class="notasortlink" href="'.DOL_URL_ROOT.'/fichinter/list.php?socid='.$object->id.'">'.$langs->trans("AllInterventions").'<span class="badge marginleftonlyshort">'.$num.'</span></td>';	// InfraS change
			...
			while ($i < $num && $i < $MAXLIST)
			{
				...
				$fichinter_static->fetch($objp->id);	// InfraS Change
                //$fichinter_static->statut = $objp->fk_statut;	// InfraS Change
	...
	if (!empty($conf->facture->enabled) && $user->rights->facture->lire)
	{
		...
		if ($resql)
		{
			...
			if ($num > 0)
			{
				...
				print '<tr class="liste_titre">';
				print '<td colspan="4"><table width="100%" class="nobordernopadding"><tr><td>'.$langs->trans("LastCustomersBills", ($num <= $MAXLIST ? "" : $MAXLIST)).'</td><td class="right"><a class="notasortlink" href="'.DOL_URL_ROOT.'/compta/facture/list.php?socid='.$object->id.'">'.$langs->trans("AllBills").'<span class="badge marginleftonlyshort">'.$num.'</span></a></td>';	// InfraS change
