...
//    if (empty($conf->global->FICHINTER_DISABLE_DETAILS))	// InfraS change
//    {														// InfraS change
        // Duration
        print '<tr><td class="titlefield">'.$langs->trans("TotalDuration").'</td>';
        print '<td>'.convertSecondToTime($object->duration, 'allhourmin', $conf->global->MAIN_DURATION_OF_WORKDAY).'</td>';	// InfraS change
        print '</tr>';
//    }														// InfraS change
...
// 	if (empty($conf->global->FICHINTER_DISABLE_DETAILS))	// modification InfraS
// 	{														// modification InfraS
		...
		if ($resql)
		{
			...
			while ($i < $num)
			{
				...
				if ($object->statut == 0 && $action == 'editline' && $user->rights->ficheinter->creer && GETPOST('line_id', 'int') == $objp->rowid)
				{
					...
					// Editeur wysiwyg
					require_once DOL_DOCUMENT_ROOT.'/core/class/doleditor.class.php';
					$doleditor = new DolEditor('np_desc', $objp->description, '', 164, 'dolibarr_notes', '', false, true, $conf->global->FCKEDITOR_ENABLE_DETAILS, ROWS_2, '90%');
					$doleditor->Create();
					print '</td>';
			...
			// Add new line
			if ($object->statut == 0 && $user->rights->ficheinter->creer && $action <> 'editline')// && empty($conf->global->FICHINTER_DISABLE_DETAILS))	// InfraS change
			{
				if (!$num)
				{
                ...
                if (empty($conf->global->FICHINTER_EMPTY_LINE_DESC)) {
                    require_once DOL_DOCUMENT_ROOT.'/core/class/doleditor.class.php';
                    $doleditor = new DolEditor('np_desc', GETPOST('np_desc', 'alpha'), '', 100, 'dolibarr_notes', '', false, true, $conf->global->FCKEDITOR_ENABLE_DETAILS, ROWS_2, '90%');
                    $doleditor->Create();
                }
                print '</td>';
	...
	if (empty($reshook))
	{
		if ($user->socid == 0)
		{
            if ($action != 'editdescription' && ($action != 'presend')) {
                // Validate
				if ($object->statut == Fichinter::STATUS_DRAFT && (count($object->lines) > 0)) {// || !empty($conf->global->FICHINTER_DISABLE_DETAILS))) {	// modification InfraS
