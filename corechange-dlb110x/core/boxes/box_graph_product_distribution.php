...
class box_graph_product_distribution extends ModeleBoxes
{
	...
	public function loadBox($max = 5)
	{
		...
		$boxWidth	= ($_SESSION['dol_screenwidth'] - 400) / 2;	// InfraS add
		$WIDTH = ($nbofgraph >= 2 || !empty($conf->dol_optimize_smallscreen)) ? $boxWidth / 2 : $boxWidth;	// InfraS change
		$HEIGHT = $WIDTH - 120;	// InfraS change
		...
		if (!empty($conf->facture->enabled) && !empty($user->rights->facture->lire))
		{
			...
			if ($showinvoicenb)
			{
				...
				if (!$mesg)
				{
					...
					while ($i <= $tot)
					{
						$data1[$i][0] = dol_trunc($data1[$i][0], 15);	// modification InfraS 5 => 15 // Required to avoid error "Could not draw pie with labels contained inside canvas"
						$legend[] = $data1[$i][0];
						$i++;
					}
					...
					$px1->setShowLegend(1);	// InfraS change
					$px1->setShowPointValue($showpointvalue);
					$px1->setShowPercent(1);	// InfraS change
		...
		if (!empty($conf->propal->enabled) && !empty($user->rights->propale->lire))
		{
			...
			if ($showpropalnb)
			{
				...
				if (!$mesg)
				{
					...
					while ($i <= $tot)
					{
						$data2[$i][0] = dol_trunc($data2[$i][0], 15);	// modification InfraS 5 => 15 // Required to avoid error "Could not draw pie with labels contained inside canvas"
						$legend[] = $data2[$i][0];
						$i++;
					}
					...
					$px2->setShowLegend(1);	// InfraS change
					$px2->setShowPointValue($showpointvalue);
					$px2->setShowPercent(1);	// InfraS change
		...
		if (!empty($conf->commande->enabled) && !empty($user->rights->commande->lire))
		{
			...
			if ($showordernb)
			{
				...
				if (!$mesg)
				{
					...
					while ($i <= $tot)
					{
						$data3[$i][0] = dol_trunc($data3[$i][0], 15);	// modification InfraS 5 => 15 // Required to avoid error "Could not draw pie with labels contained inside canvas"
						$legend[] = $data3[$i][0];
						$i++;
					}
					...
					$px3->setShowLegend(1);	// InfraS change
					$px3->setShowPointValue($showpointvalue);
					$px3->setShowPercent(1);	// InfraS change
