...
function print_left_eldy_menu($db, $menu_array_before, $menu_array_after, &$tabMenu, &$menu, $noout = 0, $forcemainmenu = '', $forceleftmenu = '', $moredata = null)
{
	...
	//		{ // InfraS change
				...
				$newmenu->add("/admin/company.php?mainmenu=home&amp;leftmenu=setup", $langs->trans("MenuCompanySetup").$warnpicto, 1); // InfraS change
				$warnpicto = '';
	...
	//			if ($usemenuhider || empty($leftmenu) || $leftmenu=="propals") { // InfraS change
     ...
     //           } // InfraS change
			...
			// Shipments	=> InfraS moved from line 1651
    ...
    //            } // InfraS change
		...
		if ($mainmenu == 'products')
		{
			...
			// Shipments	=> InfraS moved to line 999
	//		if (!empty($conf->expedition->enabled))
	//		{
	//			$langs->load("sendings");
	//			$newmenu->add("/expedition/index.php?leftmenu=sendings", $langs->trans("Shipments"), 0, $user->rights->expedition->lire, '', $mainmenu, 'sendings');
	...
	//			if ($usemenuhider || empty($leftmenu) || $leftmenu=="receptions") // InfraS change
				$newmenu->add("/reception/list.php?leftmenu=receptions&search_status=0", $langs->trans("StatusReceptionDraftShort"), 2, $user->rights->reception->lire); // InfraS change
	//			if ($usemenuhider || empty($leftmenu) || $leftmenu=="receptions") // InfraS change
				$newmenu->add("/reception/list.php?leftmenu=receptions&search_status=1", $langs->trans("StatusReceptionValidatedShort"), 2, $user->rights->reception->lire); // InfraS change
	//			if ($usemenuhider || empty($leftmenu) || $leftmenu=="receptions") // InfraS change
				$newmenu->add("/reception/list.php?leftmenu=receptions&search_status=2", $langs->trans("StatusReceptionProcessedShort"), 2, $user->rights->reception->lire); // InfraS change
		...
		if ($mainmenu == 'project')
		{
			if (!empty($conf->projet->enabled))
			{
				...
				elseif ($conf->global->PROJECT_USE_OPPORTUNITIES == 1)
				{
					...
					$newmenu->add('/projet/list.php?mainmenu=project&amp;leftmenu=projects&search_usage_opportunity=1&search_status=99&contextpage=lead', $langs->trans("ListOpenLeads"), 2, $showmode); // InfraS change
				...
				elseif ($conf->global->PROJECT_USE_OPPORTUNITIES == 2) {	// 2 = leads only
					$newmenu->add('/projet/list.php?mainmenu=project&amp;leftmenu=projects&search_usage_opportunity=1&search_status=99', $langs->trans("List"), 2, $showmode); // InfraS change
				}
