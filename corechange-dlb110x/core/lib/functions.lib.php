...
function dol_print_phone($phone, $countrycode = '', $cid = 0, $socid = 0, $addlink = '', $separ = "&nbsp;", $withpicto = '', $titlealt = '', $adddivfloat = 0)
{
	...
	if (strtoupper($countrycode) == "FR")
	{
		...
		{
			$newphone = substr($newphone, 0, 3).$separ.substr($newphone, 3, 1).$separ.substr($newphone, 4, 2).$separ.substr($newphone, 6, 2).$separ.substr($newphone, 8, 2).$separ.substr($newphone, 10, 2);	// InfraS change
		}
		elseif (dol_strlen($phone) == 13)	// InfraS add
		{	// InfraS add
...
function img_picto($titlealt, $picto, $moreatt = '', $pictoisfullpath = false, $srconly = 0, $notitle = 0, $alt = '', $morecss = '', $marginleftonlyshort = 2)
{
	...
	if ($pictoisfullpath) {
        ...
        if (empty($srconly) && in_array($pictowithouttext, array(
        		...
        		'address', 'barcode', 'bookmark', 'building', 'cash-register', 'close_title', 'cubes', 'delete', 'dolly', 'edit', 'ellipsis-h',
				...
				'note', 'setup', 'sign-out', 'split', 'switch_off', 'switch_on', 'unlink', 'uparrow', 'globe',
				'jabber', 'skype', 'twitter', 'facebook', 'linkedin',
				'instagram', 'snapchat', 'youtube', 'google-plus-g', 'whatsapp',
				'chevron-left', 'chevron-right', 'chevron-down', 'chevron-top',
				'members', 'ticket', 'generic',
        		'error', 'warning'
			)	// InfraS delete "bank tools wrench home companies products commercial invoicing accountancy project hrm title_setup title_accountancy title_bank title_hrm title_agenda"
		)) {
...
function load_fiche_titre($titre, $morehtmlright = '', $picto = 'generic', $pictoisfullpath = 0, $id = '', $morecssontable = '', $morehtmlcenter = '')
{
	...
	if ($picto) $return .= '<td class="nobordernopadding widthpictotitle valignmiddle col-picto">'.img_picto('', $picto, 'class="valignmiddle widthpictotitle pictotitle"', $pictoisfullpath).'</td>';	// InfraS delete class "opacityhigh"
	...
	if (empty($reshook))
	{
		...
		if (!empty($conf->use_javascript_ajax))
		{
			...
			if ($zone == 'private' && empty($conf->dol_use_jmobile))
			{
				print '
							/* Manage .side-nav */
							$(".side-nav").css({top: $("div#id-top").height() + "px"});
