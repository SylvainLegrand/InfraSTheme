...
function showSkins($fuser, $edit = 0, $foruserprofile = false)
{
	...
	else
	{
		...
		if ($edit)
		{
	   	...
	   	else
	   	{
			...
			if ($color) print '<input type="text" class="colorthumb" disabled="disabled" style="padding: 1px; margin-top: 0; margin-bottom: 0; color: #'.txt_color($color).'; background-color: #'.$color.'" value="'.$color.'">';	// InfraS ajout "color: #'.txt_color($color).';"
	...
	if ($foruserprofile)
	{
		...
		else
		{
			...
			if ($color)
			{
				if ($color != colorArrayToHex(colorStringToArray($colorbacklinepairhover))) print '<input type="text" class="colorthumb" disabled="disabled" style="padding: 1px; margin-top: 0; margin-bottom: 0; color: #'.txt_color($color).'; background-color: #'.$color.'" value="'.$color.'">';	// InfraS ajout "color: #'.txt_color($color).';"
				else print $langs->trans("Default");
			}
			else print $langs->trans("Default");
		}
