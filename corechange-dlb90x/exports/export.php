    ...
    */
    ...
    if (count($objexport->array_export_code))
    {
        foreach ($objexport->array_export_code as $key => $value)
        {
			...
			print img_object($objexport->array_export_module[$key]->getName(), $icon, 'style="max-height:16px;"').' ';
            print $label;
...
if ($step == 2 && $datatoexport)
{
    ...
    //print $value.'-'.$icon.'-'.$label."<br>";
    print img_object($objexport->array_export_module[0]->getName(), $icon, 'style="max-height:16px;"').' ';
    print $label;
    print '</td></tr>';
    ...
    foreach($fieldsarray as $code=>$label)
    {
        ...
        print img_object('',$entityicon, 'style="max-height:16px;"').' '.$langs->trans($entitylang);
        print '</td>';
