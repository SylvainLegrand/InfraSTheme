	...
	function loadBox($max=5)
	{
		...
		$langs->load("bills");
				...
				if (! $mesg)
				{
					...
					while ($i <= $tot)
					{
						$data1[$i][0]=dol_trunc($data1[$i][0], 4);	// modification InfraS 5 => 4 // Required to avoid error "Could not draw pie with labels contained inside canvas"
						$legend[]=$data1[$i][0];
						$i++;
					}
		...
		$langs->load("propal");
				...
				if (! $mesg)
				{
					...
					while ($i <= $tot)
					{
						$data2[$i][0]=dol_trunc($data2[$i][0], 4);	// modification InfraS 5 => 4 	// Required to avoid error "Could not draw pie with labels contained inside canvas"
						$legend[]=$data2[$i][0];
						$i++;
					}
		...
		if (! empty($conf->commande->enabled) && ! empty($user->rights->commande->lire))
		{
			...
			if ($showordernb)
			{
				...
				if (! $mesg)
				{
					...
					while ($i <= $tot)
					{
						$data3[$i][0]=dol_trunc($data3[$i][0], 4);	// modification InfraS 5 => 4 	// Required to avoid error "Could not draw pie with labels contained inside canvas"
						$legend[]=$data3[$i][0];
						$i++;
					}
