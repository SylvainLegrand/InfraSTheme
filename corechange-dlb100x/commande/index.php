...
if (! empty($conf->commande->enabled))
{
	...
	if ($resql)
	{
		$num = $db->num_rows($resql);	// Infras moved from line 193
...
$sql = "SELECT c.rowid, c.ref, c.fk_projet, c.fk_statut, c.facture, c.date_cloture as datec, c.tms as datem,";	// InfraS add ", c.fk_projet"
...
if (! empty($conf->commande->enabled))
{
	$sql = "SELECT c.rowid, c.ref, c.fk_projet, c.fk_statut, c.facture, s.nom as name, s.rowid as socid";	// InfraS add ", c.fk_projet"
    $sql.= ", s.client";
    $sql.= ", s.code_client";
    $sql.= ", s.canvas";
	$sql.=" FROM ".MAIN_DB_PREFIX."commande as c";
