	...
	elseif (array_key_exists('INV', $tmptag) && $tmptag['INV'] > 0)
	{
		...
		if ($result)
		{
			...
			if (! empty($FinalPaymentAmt) && $paymentTypeId > 0)
			{
				...
				if (! $error)
				{
					$paiement_id = $paiement->create($userpayment, 1);    //$paiement_id = $paiement->create($user, 1); // This include closing invoices and regenerating documents
					if ($paiement_id < 0)
					{
				...
				if (! $error && ! empty($conf->banque->enabled))
				{
					...
					if ($bankaccountid > 0)
					{
						...
						$result=$paiement->addPaymentToBank($userpayment,'payment',$label, $bankaccountid, '', '');	// $result=$paiement->addPaymentToBank($user,'payment',$label, $bankaccountid, '', '');
						if ($result < 0)
						{
							$postactionmessages[] = $paiement->error.' '.join("<br>\n", $paiement->errors);
							$ispostactionok = -1;
...
if ($ispaymentok)
{
    ...
    print '<span style="font-size: 24px;"><strong>'.$langs->trans("YourPaymentHasBeenRecorded")."</strong></span><br><br>\n";	// InfraS add
//    print $langs->trans("YourPaymentHasBeenRecorded")."<br>\n";	// InfraS change
    if ($TRANSACTIONID) print '<span style="font-size: 20px;">'.$langs->trans("ThisIsTransactionId",$TRANSACTIONID)."</span><br><br><br>\n";	// InfraS add
//    if ($TRANSACTIONID) print $langs->trans("ThisIsTransactionId", $TRANSACTIONID)."<br><br>\n";	// InfraS change

    $key='ONLINE_PAYMENT_MESSAGE_OK';
    if (! empty($conf->global->$key)) print '<span style="font-size: 24px;"><strong>'.$conf->global->$key."</strong></span><br><br>\n";	// InfraS add
//    if (! empty($conf->global->$key)) print $conf->global->$key;	// InfraS change
