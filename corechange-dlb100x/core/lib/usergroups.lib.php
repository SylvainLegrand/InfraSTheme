...
function show_theme($fuser, $edit = 0, $foruserprofile = false)
{
	...
	if ($foruserprofile)
	{
		...
		else
		{
			...
			if ($color)
			{
				if ($color != 'e6edf0') print '<input type="text" class="colorthumb" disabled="disabled" style="padding: 1px; margin-top: 0; margin-bottom: 0; color: #'.txt_color($color).'; background-color: #'.$color.'" value="'.$color.'">';	// InfraS ajout "color: #'.txt_color($color).';"
				else print $langs->trans("Default");
			}
			else print $langs->trans("Default");
		}
