...
if ($resql)
{
    ...
    if ($conf->use_javascript_ajax)
    {
		...
		$dolgraph->setWidth('400');	// InfraS change 100% to 400
		$dolgraph->SetHeight('250');	// InfraS add
...
{
	$sql = "SELECT f.rowid, f.ref, f.fk_projet, s.nom as name, s.rowid as socid";	// InfraS add ", f.fk_projet"
	$sql.= ", s.client";	// InfraS add
    $sql.= ", s.code_client";	// InfraS add
	...
	if ($resql)
	{
		$num = $db->num_rows($resql);	// Infras moved from line 200
		print '<table class="noborder" width="100%">';
		print '<tr class="liste_titre">';
		print '<th colspan="3">'.$langs->trans("DraftFichinter").($num?' <span class="badge">'.$num.'</span>':'').'</th></tr>';	// InfraS change colspan 2 to 3 and add .($num?' <span class="badge">'.$num.'</span>':'')
		...
		if ($num)
		{
			...
			while ($i < $num)
			{
				...
				$fichinterstatic->id=$obj->rowid;	// InfraS add
				$fichinterstatic->ref=$obj->ref;	// InfraS add

				$companystatic->id=$obj->socid;	// InfraS add
				$companystatic->name=$obj->name;	// InfraS add
				$companystatic->client=$obj->client;	// InfraS add
				$companystatic->code_client=$obj->code_client;	// InfraS add
				$companystatic->canvas=$obj->canvas;	// InfraS add
				$projectstatic->id=$obj->fk_projet;	// InfraS add
				print '<tr class="oddeven">';
				print '<td class="nowrap">';
				print $fichinterstatic->getNomUrl(1);	// InfraS add
	//			print "<a href=\"card.php?id=".$obj->rowid."\">".img_object($langs->trans("ShowFichinter"), "intervention").' '.$obj->ref."</a></td>";	// InfraS change
	//			print '<td><a href="'.DOL_URL_ROOT.'/comm/card.php?socid='.$obj->socid.'">'.img_object($langs->trans("ShowCompany"), "company").' '.dol_trunc($obj->name, 24).'</a></td></tr>';	// InfraS change
                print "</td>";	// InfraS add
                ...
                print '</td></tr>';	// InfraS add
				$i++;
			}
		}
		else	print '<tr class="oddeven"><td colspan="2" class="opacitymedium">'.$langs->trans("NoFichinterDraft").'</td></tr>';	// InfraS add
		print "</table><br>";
	}
}
...
$sql = "SELECT f.rowid, f.ref, f.fk_projet, f.fk_statut, f.date_valid as datec, f.tms as datem,";	// InfraS add ", f.fk_projet"
...
if ($resql)
{
	print '<table class="noborder" width="100%">';
	print '<tr class="liste_titre">';
	print '<th colspan="5">'.$langs->trans("LastModifiedInterventions", $max).'</th></tr>';	// InfraS change colspan 4 to 5
	...
	//		print '<td><a href="'.DOL_URL_ROOT.'/comm/card.php?socid='.$obj->socid.'">'.img_object($langs->trans("ShowCompany"), "company").' '.$obj->name.'</a></td>';	// InfraS change
			print '<td class="nowrap">';	// InfraS add
            print $companystatic->getNomUrl(1, 'company', 16);	// InfraS add
			print "</td>";	// InfraS add
			print '<td class="nowrap">';	// InfraS add
...
if (! empty($conf->ficheinter->enabled))
{
	$sql = "SELECT f.rowid, f.ref, f.fk_projet, f.fk_statut, s.nom as name, s.rowid as socid";	// InfraS add ", f.fk_projet"
	$sql.= ", s.client";	// InfraS add
	$sql.= ", s.code_client";	// InfraS add
	$sql.= ", s.canvas";	// InfraS add
	$sql.=" FROM ".MAIN_DB_PREFIX."fichinter as f";
	...
	//			print '<td><a href="'.DOL_URL_ROOT.'/comm/card.php?socid='.$obj->socid.'">'.img_object($langs->trans("ShowCompany"), "company").' '.dol_trunc($obj->name, 24).'</a></td>';	// InfraS change
				print '<td class="nowrap">';	// InfraS add
				print $companystatic->getNomUrl(1, 'company', 16);	// InfraS add
				print "</td>";	// InfraS add
				print '<td class="nowrap">';	// InfraS add
