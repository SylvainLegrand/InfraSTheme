...
if ($action == 'create' || $action == 'adduserldap')
{
	...
	print load_fiche_titre($langs->trans("NewUser"), '', 'title_user.png');	// InfraS ajout ", '', 'title_user.png'"
					...
					if (empty($reshook))
					{
						...
						if (! empty($groupslist))
						{
							foreach($groupslist as $group)
							{
								...
								if ($caneditgroup)
								{
									print '<a href="'.DOL_URL_ROOT.'/user/group/card.php?id='.$group->id.'">'.img_object($langs->trans("ShowGroup"), "group", 'class="img16"').' '.$group->name.'</a>';	// InfraS ajout ", 'class="img16"'"
								}
								else
								{
									print img_object($langs->trans("ShowGroup"), "group", 'class="img16"').' '.$group->name;	// InfraS ajout ", 'class="img16"'"
								}
								print '</td>';
