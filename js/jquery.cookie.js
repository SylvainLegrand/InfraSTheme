...
(function (factory) {
	...
	var config = $.cookie = function (key, value, options) {
		...
		var result = key ? undefined : {},	// Read	// To prevent the for loop in the first place assign an empty array in case there are no cookies at all. Also prevents odd result when calling $.cookie().
		...
		for (; i < l; i++) {
			...
			if (!key && (cookie = read(cookie)) !== undefined) {	// Prevent storing a cookie that we couldn't decode.
				result[name] = cookie;
			}
		}
		return result;
